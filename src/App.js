...
function App() {
  ...
  async function getDataFromAPI(isTeamSelected, season) {
    ...
    if (season) {
      const res = await fetch(`${apiUrl}/rankings/${driversOrTeams}?season=${season}`)
  ...
  return (
    <div className="App">
      <div style={{ backgroundColor: 'black' }}><Image src="https://e2.365dm.com/22/03/768x432/skysports-f1-its-only-live-once_5708736.jpg?20220316191225" /></div>
      <h1>Seasons</h1>
      <div>{ seasons.map(s => <Button key={`season-button-${s}`} data-testid={`season-button-${s}`} style={{ margin: '10px' }} onClick={e => handleSeasonClick(s)} variant="primary">{s}</Button>) }</div>
      Click to switch: 
      <ToggleButton
        className="mb-2"
        id="toggle-check"
      ...
      {
        <>
        { isTeams ? <TeamsTable data={teamRankings} /> : <DriversTable data={driverRankings} /> }
        </>
      }
    </div>
  );
